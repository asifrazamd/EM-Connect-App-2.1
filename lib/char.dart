...
class BLECharacteristicHelper {
  /// Searches the YAML file each time to get the characteristic name
  static Future<String> getCharacteristicName(String uuid) async {
    final normalized = uuid.toLowerCase().substring(0, 8);
    ...
    try {
      ...
      if (yaml is YamlMap && yaml['uuids'] is YamlList) {
        for (var item in yaml['uuids']) {
          ...
          if (rawUuid is int) {
            itemUuid = rawUuid.toRadixString(16);
          } else {
            itemUuid = rawUuid.toString().toLowerCase().replaceAll("0x", "");
          }

          itemUuid = itemUuid.padLeft(8, '0');

          if (itemUuid == normalized) {
            final name = item['name'].toString();
            debugPrint('[UUID FOUND] $normalized → $name');
            return name;
          }
        }
      }
    } catch (e) {
      debugPrint('Error loading YAML or parsing: $e');
    }

    debugPrint('[UUID NOT FOUND] $uuid → $normalized');
    return '$uuid :';
  }
}
